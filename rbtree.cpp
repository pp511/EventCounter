...
void RBTree::inOrderUtil(Treenode* root) {
	...
	std::cout << root->m_id << " " << root->m_count << " Color="
			<< root->m_color << std::endl;
...
void RBTree::rotateRight(Treenode* &root, Treenode* &currnode) {
	...
	else if (currnode->parent == m_treeNil){
			root = leftNode;
	}
	else{
...
void RBTree::rotateLeft(Treenode* &root, Treenode* &currnode) {
	...
	if (rightNode->right != m_treeNil){
		rightNode->right->parent = currnode;
	}
	...
	while (currnode != root && currnode->parent->m_color == RED) {
		...
		if (g->left == p){ // parent is left child
			if (u->m_color == RED){//case 1: CLRS
				g->m_color = RED;u->m_color = BLACK;p->m_color = BLACK;
				currnode = g;
			} else {
...
Treenode* RBTree::insertNode(Treenode* root, Treenode* currnode) {
	...
	if (root->m_id < currnode->m_id) {
		...
		// For genericity.This condition must never reach in out context.
...
void RBTree::Increase(int key, int count) {
	...
	if (toIncrease == NULL) {
		...
		//Update the root of the RBtree.
		m_root->parent = m_treeNil;
		//Fix the violations of the RBTree properties.
...
void RBTree::deleteFixup(Treenode* &root, Treenode* &currnode) {
	while (currnode != root && currnode->m_color == BLACK) {
		...
		Treenode* s = NULL;//Sibling pointer
		if (currnode = p->left) {
			s = p->right;
			if (s->m_color == RED){ // case 1
				s->m_color = BLACK;p->m_color = RED;
				rotateLeft(root, p);
				s = p->right;
			}
			if (s->left->m_color == BLACK && s->right->m_color == BLACK){ //case 2
				...
				s->m_color = p->m_color = BLACK;//Case 4
				s->right->m_color = BLACK;
				rotateLeft(root, p);
				currnode = root;
			}
		} else{ // symmetric case if Currentnode is right child of its parent, symmetric to left case.
			s = p->left;
			if (s->m_color == RED) {
				s->m_color = BLACK;p->m_color = RED;
				rotateRight(root, p);
...
void RBTree::deleteNode(Treenode* &root, Treenode* &toDelete) {
	...
	Treenode* child = willbeDeleted->left == m_treeNil ?
					willbeDeleted->right : willbeDeleted->left;

	child->parent = willbeDeleted->parent;
	if (willbeDeleted->parent == m_treeNil){
		root = child;
	}
...
/*
 *
 *This function doesn't take linear time hencce not used here.
 /*
